...
import LoginPage from '../../pageobjects/android/login-page.js';
import ProductsPage from '../../pageobjects/android/products-page.js';
...
Then(/^I should redirect to products page$$/, async() => {
    ...
    await expect(ProductsPage.header).toHaveAttribute('text',MESSAGE.STANDARD.message)
});
...
Then(/^I should get error message for invalid creditials$/, async() => {
    ...
    await expect(LoginPage.errorMsg).toHaveAttribute('text',MESSAGE.NO_MATCH.message)
});
...
Then(/^I should get error message for locked creditials$/, async() => {
	await expect(LoginPage.errorMsg).toBeExisting();
    await expect(LoginPage.errorMsg).toHaveAttribute('text',MESSAGE.LOCKED.message)
});
...
Then(/^I should get error message for no password$/, async() => {
	await expect(LoginPage.errorMsg).toBeExisting();
    await expect(LoginPage.errorMsg).toHaveAttribute('text',MESSAGE.NO_PASSWORD.message)
});
...
Then(/^I should get error message for no username$/, async() => {
	await expect(LoginPage.errorMsg).toBeExisting();
    await expect(LoginPage.errorMsg).toHaveAttribute('text',MESSAGE.NO_USER_DETAILS.message)
});
